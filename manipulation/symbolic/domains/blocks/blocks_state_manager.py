...
class BlocksStateManager(BaseStateManager):
    ...
    def _check_xy_overlap(
        self, block1_idx: int, block2_idx: int, threshold: float = 0.8
    ) -> bool:
        """
    ...
    def sample_random_state(
        ...
        selected_blocks = list(
            np.random.choice(self.GRASPABLE_BLOCKS, n_blocks, replace=False)
        )

        if include_platforms:
            selected_blocks.extend(self.PLATFORM_BLOCKS[:1])
        ...
        for block_idx in selected_blocks:
            ...
            for _ in range(max_attempts):
                ...
                for px, py, pw, pd in placed_blocks:
                    ...
                    if (
                        abs(x - px) < (width + pw) / 2 + clearance
                        and abs(y - py) < (depth + pd) / 2 + clearance
                    ):
                        collision = True
                        break
            ...
            if not placed:
                print(
                    f"Warning: Could not place block_{block_idx} without collision after {max_attempts} attempts"
                )
    ...
    def compute_stack_pose(
        self, target_block_idx: int, block_to_place_idx: int
    ) -> Tuple[np.ndarray, np.ndarray]:
        """
    ...
    def compute_table_pose(
        self, x: float, y: float, block_idx: int
    ) -> Tuple[np.ndarray, np.ndarray]:
        """
