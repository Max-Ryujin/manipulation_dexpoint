...
class FrankaEnvironment(BaseEnvironment):
    ...
    def rest(self, duration: float):
        dt_per_step = self.model.opt.timestep * self.frame_skip
        steps = int(duration / dt_per_step)
    ...
    def check_collisions(self) -> bool:
        ...
        for i in range(self.data.ncon):
            ...
            if (
                body1 in self._collision_exception_ids
                or body2 in self._collision_exception_ids
            ):
                continue
    ...
    def is_path_collision_free(
        self, config1: np.ndarray, config2: np.ndarray, steps: int = 5
    ) -> bool:
    ...
    def set_object_pose(self, body_name: str, pos: np.ndarray, quat: np.ndarray = None):
        ...
        if joint_id >= 0:
            ...
            self.data.qvel[
                self.model.jnt_dofadr[joint_id] : self.model.jnt_dofadr[joint_id] + 6
            ] = 0
